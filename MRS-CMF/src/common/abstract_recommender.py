...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        self.v_feat, self.t_feat, self.f_feat = None, None, None
        fused_feat_file_path = "D:\Python_project2\DA-MRS-main_revised\data\pre_embeddings\pre_sports\image_feat.npy"
        if not config['end2end'] and config['is_multimodal_model']:
            ...
            if os.path.isfile(t_feat_file_path):
                self.t_feat = torch.from_numpy(np.load(t_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)
            if os.path.isfile(fused_feat_file_path):
                self.f_feat = torch.from_numpy(np.load(fused_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                    self.device)

            assert self.v_feat is not None or self.t_feat is not None or self.f_feat is not None, 'Features all NONE'
