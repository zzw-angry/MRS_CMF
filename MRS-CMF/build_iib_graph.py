...
def gen_item_matrix(all_edge, no_items, mu=0.1):
    edge_dict = defaultdict(set)

    for edge in all_edge:
        user, item, timestamp = edge
        edge_dict[item].add((user, timestamp))
    ...
    for head in range(len(key_list)):
        ...
        for rear in range(head+1, len(key_list)):
            ...
            common_users = set(user for user, _ in item_head).intersection(user for user, _ in item_rear)
            weighted_inter_len = 0
            inter_len = len(item_head.intersection(item_rear))
            for user in common_users:
                time_head = [timestamp for u, timestamp in item_head if u == user]
                time_rear = [timestamp for u, timestamp in item_rear if u == user]
                for t1 in time_head:
                    for t2 in time_rear:
                        delta_t = abs(t1 - t2)
                        weight = math.exp(-mu * delta_t)
                        weighted_inter_len += weight

            if inter_len >= 2:
                item_graph_matrix[head_key-min_item][rear_key-min_item] = weighted_inter_len
                item_graph_matrix[rear_key-min_item][head_key-min_item] = weighted_inter_len
    bar.close()
...
if __name__ == 	'__main__':
    ...
    train_data = train_df[[uid_field, iid_field, time_field]].to_numpy()
    # item_item_pairs =[]
    ...
    for i in range(num_item):
        if item_num[i] <= top_k:
            ...
            edge_list = [edge_list_i, edge_list_j]
            item_graph_dict[i] = edge_list
    name = 'item_graph_dict_del0.2_mu0.1.npy'
